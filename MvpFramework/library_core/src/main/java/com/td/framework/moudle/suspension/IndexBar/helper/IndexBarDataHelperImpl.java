package com.td.framework.moudle.suspension.IndexBar.helper;
...
public class IndexBarDataHelperImpl implements IIndexBarDataHelper {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                if (target != null) {
                   // L.e(target);
                    ...
                    indexPinyinBean.sortToken.wholeSpell = pySb.toString();
    ...
    public IIndexBarDataHelper fillInexTag(List<? extends BaseIndexPinyinBean> datas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (indexPinyinBean.isNeedToPinyin()) {
                ...
                try {
                    tagString = indexPinyinBean.getBaseIndexPinyin().toString().substring(0, 1);
                    if (tagString!=null && tagString.matches("[A-Z]")) {//如果是A-Z字母开头
                        indexPinyinBean.setTopTag(tagString);
                        indexPinyinBean.sortToken.simpleSpell = tagString;
                    } else {//特殊字母这里统一用#处理
                        indexPinyinBean.setTopTag("#");
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    indexPinyinBean.setTopTag("#");
        ...
        Collections.sort(datas, new Comparator<BaseIndexPinyinBean>() {
            ...
            public int compare(BaseIndexPinyinBean lhs, BaseIndexPinyinBean rhs) {
                if (!lhs.isNeedToPinyin()) {
                    ...
                    return 0;
                } else if (lhs.getTopTag().equals("#")) {
                    return 1;
                } else if (rhs.getTopTag().equals("#")) {
                    return -1;
                } else {
    ...
    public IIndexBarDataHelper getSortedIndexDatas(List<? extends BaseIndexPinyinBean> sourceDatas, List<String> indexDatas) {
        ...
        String baseIndexTag;
        for (int i = 0; i < size; i++) {
            baseIndexTag = sourceDatas.get(i).getTopTag();
