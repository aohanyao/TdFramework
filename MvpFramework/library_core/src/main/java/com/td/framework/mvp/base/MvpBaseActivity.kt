 ...
 *
 * Github https://github.com/aohanyao
 */

abstract class MvpBaseActivity<P> : TDBaseActivity(), DialogInterface.OnCancelListener, DialogHelper.OnDialogConfirmListener {
    ...
    protected fun onDialogCancel() {
        ...
        if (getP() != null) {
            if (checkIsPresenter()) {
                (getP() as BasePresenter<*>).unSubscribe()
            }
        }
        if (subscribe != null) {
            subscribe!!.dispose()
