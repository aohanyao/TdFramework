...
public class FlowTagLayout extends ViewGroup {
     ...
     * 单行模式下位置
     */
    private int mSingleLineChildSize = -1;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0, childCount = getChildCount(); i < childCount; i++) {
            ...
            if ((lineWidth + realChildWidth) > sizeWidth) {
                if (isSingleLine) {
                    if (mSingleLineChildSize == -1) {
                        mSingleLineChildSize = i - 1;
                    }
                    continue;
                }
                //换行
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (isSingleLine && mSingleLineChildSize != -1) {
            count = mSingleLineChildSize;
        }
        //遍历子控件，记录每个子view的位置
        for (int i = 0, childCount = count; i < childCount; i++) {
            View childView = getChildAt(i);
    ...
    private void reloadData() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            final View childView = mAdapter.getView(i, this, this);
//            addView(childView,
